...
devtools::install_github("shinyGlobe", "trestletech")
library(shinyGlobe)
Install_github repo trestletech/shinyGlobe
...
shinyServer(function(input, output, session) {
  ...
  country_data <- reactive({
             ...
             output$totLoss <- renderValueBox({
                                 valueBox("$ 30 Billion", "Total Interventions Cost",icon = icon("bank"), color = 'purple') })
            ...
            output$plot1 <- renderPlotly({
                            ...
                            p <- plot_geo(attack_freq_country,height = 800) %>%
                                   add_trace(
                                         ...
                                         text = ~paste(paste("Country:",COUNTRY),paste("Total IRS:",FREQ),
                                                       paste("Total ITN Distributed:", DEATH),sep = "<br />"), locations = ~CODE,
                                   ...
                                   layout(
                                          title = 'Global Malaria Interventions Between 2010 - 2019',
